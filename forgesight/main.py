...
@bot.tree.command(name="message_length", description="Set the message size requirements for earning gold")
@is_mod_or_admin()
async def set_messagelength(Interaction: discord.Interaction, min_message_length: int):
    ...
    logger.info(f"Minimum message length set to {MIN_MESSAGE_LENGTH} by {Interaction.user}")
    
@bot.tree.command(name="subscriber_bonus", description="Set the subscriber bonus applied to Gold rewards.")
@is_mod_or_admin()
async def set_subscriberbonus(Interaction: discord.Interaction, subscriber_bonus: int):
    ...
    if subscriber_bonus < 0:
        await Interaction.response.send_message(f"Invalid subscriber bonus: {subscriber_bonus}. Subscriber bonus must be a positive integer.", ephemeral=True)
        logger.warning(f"Invalid subscriber bonus entered by {Interaction.user} for /forgesight subscriberbonus command.")
        return
...
@bot.tree.command(name="media_bonus", description="Set the media bonus applied to Gold rewards.")
@is_mod_or_admin()
async def set_subscriberbonus(Interaction: discord.Interaction, media_bonus: int):
    global MEDIA_BONUS
    if media_bonus < 0:
        await Interaction.response.send_message(f"Invalid media bonus: {media_bonus}. Media bonus must be a positive integer.", ephemeral=True)
        logger.warning(f"Invalid media bonus entered by {Interaction.user} for /forgesight media_bonus command.")
        return
    MEDIA_BONUS = media_bonus
    await Interaction.response.send_message(f"Media bonus for Gold reward has been set to {SUBSCRIBER_BONUS}.", ephemeral=False)
...
async def toggle_rewards(Interaction: discord.Interaction, toggle: str):
    ...
    if toggle == "on":
        ...
        logger.warning(f"Invalid option entered by {Interaction.user} for /forgesight rewards command.")
        
@bot.tree.command(name="reward_timeout", description="Set the timeout for Gold rewards in seconds between messages.")
@is_mod_or_admin()
async def set_reward_timeout(Interaction: discord.Interaction, timeout: int):
    global MESSAGE_REWARD_TIMEOUT
    if timeout < 0:
        await Interaction.response.send_message(f"Invalid timeout: {timeout}. Timeout must be a positive integer.", ephemeral=True)
        logger.warning(f"Invalid timeout entered by {Interaction.user} for /forgesight reward_timeout command.")
        return
    MESSAGE_REWARD_TIMEOUT = timeout
    await Interaction.response.send_message(f"Message reward timeout set to {MESSAGE_REWARD_TIMEOUT} seconds.", ephemeral=False)
    logger.info(f"Message reward timeout set to {MESSAGE_REWARD_TIMEOUT} by {Interaction.user}")
...
async def commands_list(Interaction: discord.Interaction):
    ...
    print(f'{Interaction.user} used the retrieved the command list.') #! Debug print statement
    logger.info(f"{Interaction.user} used the retrieved the command list.")
    

@bot.tree.command(name="gold_leaderboard", description="Show the Server Gold leaderboard. \n Usage: /gold_leaderboard")
async def get_leaderboard(interaction: discord.Interaction):
    try:
        with open('forgesight_vault.json', 'r') as file:
            data = json.load(file)
        leaderboard_data = sorted(data.items(), key=lambda x: x[1]['gold'], reverse=True)
        ...
        logger.info("File Not Found Error:The leaderboard data file could not be found.")
    except Exception as e:
        await interaction.response.send_message(f"An error occurred: {str(e)}", ephemeral=True)
        logger.info(f"An error occurred: {str(e)}")
        
@bot.tree.command(name="gold_balance", description="Check a user's gold balance. \n Usage: /gold_balance <@user>")
async def check_gold(interaction: discord.Interaction, member: discord.Member):
    try:
        ...
        await interaction.response.send_message(embed=embed)
        print(f"{interaction.user} retrieved {member.display_name}'s gold balance.") #! Debug print statement
        logger.info(f"{interaction.user} retrieved {member.display_name}'s gold balance.")
...
async def on_message(event):
    ...
    if REWARDS == True: 
        ...
        if event.channel.id not in ALLOWED_CHANNEL_IDs:
            print(f'Message not in allowed channel {event.channel.id}') #! Debug print statement, do not log this event due to spam
            return
        ...
        if MIN_MESSAGE_REQUIRED: 
            ...
            if len(event.content) >= MIN_MESSAGE_LENGTH:
                ...
                if any(role.id == SUBSCRIBER_ROLE_ID for role in event.author.roles):
                    ...
                    print(f"User {event.author.mention} has supporter role: Applying supporter bonus, new gold reward is {current_gold_award}") #! Debug print statement
                # If the message has attachments, apply the media bonus
                if event.attachments:
                    current_gold_award *= MEDIA_BONUS
                    print(f"Message has attachments: Applying media bonus, new gold reward is {current_gold_award}") #! Debug print statement
                ...
                if time.time() - user_data["last_earned"] < MESSAGE_REWARD_TIMEOUT:
                    print(f"{event.author.mention} has already earned gold in the past {MESSAGE_REWARD_TIMEOUT} seconds.") #! Debug print statement
                    return
                # Award the gold and update the vault with new gold and last earned time
                user_data["gold"] += current_gold_award
                print(f'Message was length {len(event.content)}: Awarding {current_gold_award} gold to {user_id}') #! Debug print statement
                user_data["last_earned"] = time.time()
                print(f'Setting last earned time to {user_data["last_earned"]}') #! Debug print statement
                vault[user_id] = user_data
                logger.info(f'Awarding {current_gold_award} gold to {user_id} and updating last earned time to {user_data["last_earned"]}')
...
async def grant_gold(Interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    async with aiofiles.open('forgesight_vault.json', 'w') as file:
        await file.write(json.dumps(vault, indent=4))
    await Interaction.response.send_message(f"{amount} gold granted to {user.mention}.", ephemeral=False)
    logger.info(f"{amount} gold granted to {user} by {Interaction.user}")


@bot.tree.command(name="deduct_gold", description="Deduct gold from a user\n Usage: /deduct_gold <@user> <amount of gold>")
@is_mod_or_admin()
async def deduct_gold(Interaction: discord.Interaction, user: discord.Member, amount: int):
    ...
    if user_data["gold"] < amount:
        await Interaction.response.send_message(f"{user.mention} does not have enough gold to deduct {amount} gold.", ephemeral=False)
        return
    ...
    async with aiofiles.open('forgesight_vault.json', 'w') as file:
        await file.write(json.dumps(vault, indent=4))
    await Interaction.response.send_message(f"{amount} gold deducted from {user.mention}.", ephemeral=False)
...
@bot.tree.command(name="default_gold_award", description="Set the gold reward per message.\n Usage: /gold_reward <number of gold>")
@is_mod_or_admin()
...
@bot.tree.command(name="configuration", description="Displays the current Forgesight configuration. \n Usage: /configuration")
@is_mod_or_admin()
async def get_configuration(Interaction: discord.Interaction):
    ...
    embed.add_field(name="MIN_MESSAGE_REQUIRED", value=str(MIN_MESSAGE_REQUIRED), inline=False)
    embed.add_field(name="MIN_MESSAGE_LENGTH", value=str(MIN_MESSAGE_LENGTH), inline=False)
    embed.add_field(name="REWARDS", value=str(REWARDS), inline=False)
    embed.add_field(name="MESSAGE_REWARD_TIMEOUT", value=str(MESSAGE_REWARD_TIMEOUT), inline=False)
    embed.add_field(name="SUBSCRIBER_ROLE_ID", value=str(SUBSCRIBER_ROLE_ID), inline=False)
    ...
    embed.add_field(name="ALLOWED_CHANNEL_IDs", value=str(ALLOWED_CHANNEL_IDs), inline=False)
    await Interaction.response.send_message(embed=embed)
