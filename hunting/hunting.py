...
class Hunting(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        default_user = {"score": {}, "total": 0, "fastest_shot": 0}
    ...
    @hunting.command()
    async def leaderboard_times(self, ctx, global_leaderboard=False):
        """
        ...
        async with ctx.typing():
            sorted_acc = sorted(userinfo.items(), key=lambda x: (x[1]["fastest_shot"]), reverse=True)[:50]
        ...
        for account in sorted_acc:
            ...
            if isinstance(user_obj, discord.User) and len(str(user_obj)) > 28:
                ...
                user_name = str(user_obj)
            if user_obj == ctx.author:
                temp_msg += f"{humanize_number(account[1]['fastest_shot']) + '   ': <{score_len + 4}} <<{user_name}>>\n"
            else:
                temp_msg += f"{humanize_number(account[1]['fastest_shot']) + '   ': <{score_len + 4}} {user_name}\n"
    ...
    async def score(self, ctx, member: discord.Member = None):
        ...
        shot_time = await self.config.user(member).fastest_shot()
        total = 0
        kill_list = []
        if not score:
    ...
    async def _add_score(self, guild, author, avian, time_for_bang):
        user_data = await self.config.user(author).all()
        try:
            user_data["score"][avian] += 1
            if user_data["fastest_shot"] > time_for_bang: 
                user_data["fastest_shot"] = time_for_bang
        except KeyError:
            user_data["score"][avian] = 1
            user_data["total"] += 1
            user_data["fastest_shot"] = time_for_bang
    ...
    async def _latest_message_check(self, channel):
        ...
        async for message in channel.history(limit=5):
            ...
            delta = utc_now - message.created_at
    ...
    async def _wait_for_bang(self, guild, channel):
        ...
        if random.randrange(0, 17) > 1:
            await self._add_score(guild, author, animal, time_for_bang)
