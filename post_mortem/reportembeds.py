...
class ReportEmbeds:
    ...
    def report_embed(self):
        self.embed.set_author(name=self.user.display_name, icon_url=self.user.display_avatar.url)
        self.embed.add_field(name="Subject", value=self.user.mention, inline=False)
        self.embed.add_field(name="Death Progress", value=f"{self.user_data['progress_bar']} {self.user_data['progress'] * 100:.1f}%", inline=False)
        self.embed.add_field(name="Subject Risk Factors", value=f"{self.user_data['risk_factor']}", inline=False)
