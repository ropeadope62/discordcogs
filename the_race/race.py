...
from .animals import Animal, racers, specials, slowboys
...
class Race(commands.Cog):
    ...
    def __init__(self):
        self.config = Config.get_conf(self, 111111111, force_registration=True)
    ...
    async def mode(self, ctx, mode: str):
        ...
        Special Mode:
            All racers are special characters.
            

        Slowboys Mode:
            All racers have stat value slow or abberant.

        """

        if mode.lower() not in ("zoo", "special",  "normal", "slowboys"):
            return await ctx.send("Must select either `zoo`, 'special', 'slowboys', or `normal` as a mode.")

        if mode.lower() not in ("zoo", "special",  "normal", "slowboys"):
            return await ctx.send("Must select either `zoo`, 'special', `normal`, or 'slowboys' as a mode.")
    ...
    async def distribute_prizes(self, ctx, settings):
        ...
        if settings["Pooling"] and len(self.players[ctx.guild.id]) > 3 and len(self.players[ctx.guild.id]) < 8:
        ...
        if len(self.players[ctx.guild.id]) > 8:
            settings["Prize"] = 10000
    ...
    async def _game_setup(self, ctx):
        ...
        if mode == "zoo":
            ...
            players = [(Animal(*random.choice(specials)), user) for user in users]
            if len(players) == 1:
                players.append((Animal(*random.choice(specials)), ctx.bot.user))

        elif mode == "slowboys":
            players = [(Animal(*random.choice(slowboys)), user) for user in users]
            if len(players) == 1:
                players.append((Animal(*random.choice(slowboys)), ctx.bot.user))                
    ...
    async def run_game(self, ctx):
        ...
        setup = "\u200b\n" + "\n".join(
            f"🏁 **{animal.current}** 🚩[{jockey.display_name}]" for animal, jockey in players
        )
        ...
        while not all(animal.position == 0 for animal, jockey in players):
            ...
            for animal, jockey in players:
                if animal.position == 0:
                    fields.append(f"🏁 **{animal.current}** 🚩  [{jockey.display_name}]")
                    continue
                animal.move()
                fields.append(f"🏁 **{animal.current}** 🚩  [{jockey.display_name}]")
