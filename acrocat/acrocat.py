        ...
        for index, (author, acronym) in enumerate(self.responses.items(), start=1):
            ...
            try:
                ...
                if 1 <= vote <= len(self.responses):
                    ...
                    await ctx.send(
                        f"{response.author.display_name}'s vote has been recorded."
                    )
                else:
                    await ctx.send(
                        f"Invalid vote. Please enter a number between 1 and {len(self.responses)}."
                    )
            ...
            for index, (author, votes) in enumerate(self.votes.items(), start=1):
                embed.add_field(
                    name=f"{index}.",
                    value=f"{author.display_name}: {votes}",
                    inline=False,
                )
